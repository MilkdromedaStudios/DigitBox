...
export default function ProjectRunner({ html, title, slug, notFound }) {
  ...
  const fullDocument = html.includes("<html")
    ? html
    : `<!doctype html><html><head><meta charset=\"utf-8\"/><meta name=\"viewport\" content=\"width=device-width,initial-scale=1\"/><title>${title || "Project"}</title></head><body>${html}</body></html>`;
...
export async function getServerSideProps({ params }) {
  ...
  if (res.status === 404) return { props: { html: "", title: slug, slug, notFound: true } };
  if (!res.ok) return { props: { html: "<h1>Could not load project.</h1>", title: "Project Error", slug, notFound: false } };
