...
export default function ProfilePage() {
  ...
  function handleAvatarUpload(e) {
    ...
    if (file.size > TEN_MB) {
      setMessage("Image is too large. Maximum size is 10MB.");
      return;
    }
  ...
  return (
    <div className="content">
      ...
      <p>Customize how other players see you. All preferences are saved in your browser cookie.</p>

      <form className="post-form" style={{ maxWidth: 620 }}>
        <label>
          Optional username
          <input
        ...
        <label>
          How other players should see you
          <input
            className="auth-input"
            value={prefs.identityLabel}
            onChange={(e) => updateField("identityLabel", e.target.value)}
      ...
      <div className="section" style={{ maxWidth: 620 }}>
        ...
        <div className="profile-box">
          <img
            src={prefs.avatarDataUrl || "https://ui-avatars.com/api/?name=Player&background=333&color=fff"}
            alt="Avatar preview"
            className="profile-avatar"
          />
          <div className="profile-text">
            <span className="profile-name">{previewName}</span>
            {prefs.identityLabel && <span className="admin-badge">{prefs.identityLabel}</span>}
          </div>
        </div>
      </div>
