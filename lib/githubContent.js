...
async function getFileSha(owner, repo, branch, path) {
  const res = await fetch(`${GITHUB_API}/repos/${owner}/${repo}/contents/${encodeURIComponent(path)}?ref=${branch}`, { headers: authHeaders() });
  const res = await fetchWithRetry(`${GITHUB_API}/repos/${owner}/${repo}/contents/${encodeURIComponent(path)}?ref=${branch}`, { headers: authHeaders() }, { retries: 3, timeoutMs: 12000 });
...
export async function upsertRepoFile({ path, content, message }) {
  ...
  let res;
  try {
    res = await fetchWithRetry(`${GITHUB_API}/repos/${owner}/${repo}/contents/${encodeURIComponent(path)}`, {
    method: "PUT",
