...
# Cave interiors are deliberately near-black so excavated space reads immediately.
...
'''
if anchor in s and 'function surfaceBridgeAt(' not in s: s = s.replace(anchor, helpers + anchor)
...
if needle in s and 'freshCutsRef' not in s: s = s.replace(needle, needle + '\n  const freshCutsRef = useRef([]);')
...
if oldcut in s: s = s.replace(oldcut, newcut, 1)
...
if oldrim in s: s = s.replace(oldrim, newrim)
...
if city_anchor in s and 'Auto-build timber bridges' not in s: s = s.replace(city_anchor, bridge_draw + city_anchor, 1)
